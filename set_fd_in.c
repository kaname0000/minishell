...
/*   Updated: 2025/03/26 16:54:46 by okaname          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	set_fd_in(t_command **cmd, t_token *token)
{
	...
	while ((token[i]).value == NULL)
	{
		if ((token[i]).type == TOK_PIPE)
		{
			...
			open_file = can_access((token[i + 1]).value, &error_flag, open_file,
					&last_in);
		else if ((token[i]).type == TOK_HEREDOC)
			here_doc((token[i + 1]).value, &last_in, &(cmd[pipe_count]->fd_in));
		i++;
	}
	if (error_flag)
